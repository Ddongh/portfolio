...
def perform_rnn_analysis(code, method, start, end):
    ...
    for column in ['Close', 'Volume']:
        ...
        # 모델 학습
        model.fit(X_train.reshape(-1, 1, 1), y_train, epochs=3, batch_size=1, verbose=0)
        ...
        # 마지막 데이터를 기반으로 다음 7일의 데이터 예측
        last_data = data_scaled[-1]  # 마지막 데이터
        prediction_period = 7  # 예측 기간 (7일)
        ...
        # 예측 결과를 output_data에 추가 
        output_data[column + "_predict"] = predictions + predicted_data.flatten().tolist()
